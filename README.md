...
### 🗓 任务计划（Windows 计划任务）
在图形界面内可直接创建、管理定时任务，无需手写 `schtasks` 命令。

- 支持按“每天 / 每周 / 每月”触发
- 支持设置任务超时限制与执行后自动退出
- 支持任务列表同步、启用、禁用、删除
- 自动生成启动参数（如 `-t N -e`）
- 本地缓存任务状态，减少重复读取系统任务计划
...
- 日常任务（送礼、据点兑换、委托转交、收信用、收集线索、买卖货、日常奖励等）
- 运送委托接取（按券种 + 金额区间筛选自动抢单）
- 自动送货（路径识别、分叉测试、完整循环测试）
- 仓库物品转移（发货仓 -> 收货仓，一键存放）
- 毕业基质识别（按 `assets/weapon_data.csv` 识别并锁定）
...
- 系统：Windows
- 游戏分辨率：推荐 16:9，最低 1280×720 最佳1920x1080
- 语言：当前部分功能仅支持简体中文
...
- `src/tasks/`：核心任务逻辑（一次性任务 + 触发式任务）
- `src/ui/`：自定义 UI（含任务计划页）
- `src/scheduler/`：Windows 计划任务辅助模块
- `src/essence/`：基质识别与词条匹配
- `src/data/`：地图、角色、物品与中英文映射数据
- `assets/`：模板图、OCR相关素材、武器词条数据
- `configs/`：各任务默认配置、UI 配置与任务计划缓存
- `tests/`：单元测试/功能测试脚本
